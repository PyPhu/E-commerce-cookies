    ...
    try{
        ...
        const metadata = {
            ...
            cart_data: JSON.stringify(cart.map(item => ({
                ...
                toppings: item.flavors ?? [], // หรือส่ง toppings ตามโครงสร้างของคุณ
                quantity: item.quantity
            })))
        };
...
app.post("/webhook", express.raw({type: 'application/json'}), async (req,res) => {
    ...
    if (event.type === 'checkout.session.completed'){
        ...
        console.log(`🔔 ตรวจพบการชำระเงินสำเร็จจากคุณ: ${customer_name} (${customer_email})`); //check F12

        try{
            //save customer_it to customer in supabase
            ...
            //move items from metadata to order_items table
            ...
            // remove items from cart_item table
            await supabase
                .from('cart_items')
                .delete()
